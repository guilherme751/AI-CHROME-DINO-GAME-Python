...
class NeuralNetwork(KeyClassifier):
    ...
    def keySelector(self, distance, obHeight, 
                    speed, obType, nextObDistance, 
                    nextObHeight, nextObType):
        X = [distance, obHeight, speed, obType, nextObDistance, nextObHeight, nextObType]
        
        output = feedForward(X, self.weights[0], self.weights[1])
...
def evaluate_population(population):
    val_pop = []
    for p in population:
        aiPlayer = NeuralNetwork(p)
        _, res = manyPlaysResults(3)
        val_pop.append((res, p))
    return val_pop

def elitism(val_pop, elite_ratio):
    n = math.floor(len(val_pop) * elite_ratio)
    if n < 1: n = 1
    elite = sorted(val_pop, key=first, reverse=True)[:n]
    return [weights for (_, weights) in elite], elite[0]

def selection(val_pop, n):
    values = [val for (val, _) in val_pop] 
    prob_values = values/sum(values)   
    return np.random.choice(val_pop, p=prob_values, replace=False, size=n)


def crossover(parent1, parent2):
    rh = np.random.randint(0, len(parent1[0]) - 1)
    ro = np.random.randint(0, len(parent1[1]) - 1)

    child1 = np.concatenate((parent1[:rh], parent2[rh:]))
    child2 = np.concatenate((parent1[:ro], parent2[ro:]))
    return child1, child2

def crossover_step(pop, crossover_ratio):
    new_pop = []
...
def mutation(indiv):
    ...
    total_size = individual[0].size + individual[1].size
    rand = random.randint(0, len(total_size) - 1)
    if rand < individual[0].size:
        individual[0][rand] = np.random.random(1)[0]
    else:
        individual[1][rand%individual[0].size] = np.random.random(1)[0]    
...
def geneticAlgorithm(elite_ratio, crossover_ratio, mutation_ratio, 
                     max_Time, pop_size = 100, max_Itr = 1000):
    start = time.process_time()
    end = 0
    iter = 0
    ...
    while not conv and iter < max_Itr and end-start <=max_Time:
        
        val_pop = evaluate_population(pop)
        new_pop, best = elitism(val_pop, elite_ratio)
        ...
        crossed = crossover_step(selected, crossover_ratio)
        mutated = mutation_step(crossed, mutation_ratio)
    ...
    return best_score, best_solution, iter, conv
